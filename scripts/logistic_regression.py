# logistic_regression.py (Updated)
import os
import pandas as pd
import numpy as np
...
with open(os.path.join(models_dir, 'logistic_regression_model.pkl'), 'wb') as f:
    pickle.dump(lr, f)
print("Saved Logistic Regression model to 'models/logistic_regression_model.pkl'")
...
plt.ylabel('Actual')
plt.savefig(os.path.join(confusion_matrices_dir, 'confusion_matrix_logistic_regression_full.png'))
plt.close()
...
results.to_csv(os.path.join(results_dir, 'logistic_regression_performance.csv'), index=False)
print("Results saved to 'results/logistic_regression_performance.csv'")
...
ax.set_title('Cross-Validation Performance of Logistic Regression', fontsize=14, pad=15)
...
plt.tight_layout()
plt.savefig(os.path.join(plots_dir, 'logistic_regression_performance_bar_plot_cv.png'))
plt.close()
