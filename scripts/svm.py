...
with open(os.path.join(models_dir, 'svm_tfidf_model.pkl'), 'wb') as f:
    pickle.dump(svm, f)
print("Saved SVM model to 'models/svm_tfidf_model.pkl'")
...
plt.ylabel('Actual')
plt.savefig(os.path.join(confusion_matrices_dir, 'confusion_matrix_svm_full.png'))
plt.close()
...
results.to_csv(os.path.join(results_dir, 'svm_performance.csv'), index=False)
print("Results saved to 'results/svm_performance.csv'")
...
ax.set_title('Cross-Validation Performance of SVM', fontsize=14, pad=15)
...
plt.tight_layout()
plt.savefig(os.path.join(plots_dir, 'svm_performance_bar_plot_cv.png'))
plt.close()
