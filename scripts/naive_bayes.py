...
for fold, (train_idx, val_idx) in enumerate(kfold.split(X_tfidf)):
    ...
    # Evaluate
    y_pred_fold = nb.predict(X_val_fold)
...
with open(os.path.join(models_dir, 'naive_bayes_model.pkl'), 'wb') as f:
    pickle.dump(nb, f)
print("Saved Naive Bayes model to 'models/naive_bayes_model.pkl'")
...
plt.ylabel('Actual')
plt.savefig(os.path.join(confusion_matrices_dir, 'confusion_matrix_naive_bayes_full.png'))
plt.close()
...
results.to_csv(os.path.join(results_dir, 'naive_bayes_performance.csv'), index=False)
print("Results saved to 'results/naive_bayes_performance.csv'")
...
ax.set_title('Cross-Validation Performance of Naive Bayes', fontsize=14, pad=15)
...
plt.tight_layout()
plt.savefig(os.path.join(plots_dir, 'naive_bayes_performance_bar_plot_cv.png'))
plt.close()
